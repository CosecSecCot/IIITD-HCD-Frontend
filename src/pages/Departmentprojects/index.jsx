        ...
        <div className="relative border border-neutral-500/30">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project, index) => {
              ...
              return isExpanded ? (
                <div key={project.id} className="col-span-3 bg-[#096964] text-white p-6 transition-all duration-300 ease-in-out">
                  <div className="flex gap-6">
                    <img src={project.imageUrlExpanded || project.imageUrl} alt={project.title} className="w-1/3 object-cover" />
                    <div className="flex flex-col">
                      <h3 className="text-xl font-bold mb-2 font-anybody">{project.title}</h3>
                      <div className="flex gap-12 text-sm mb-2 font-anybody">
                        <div>
                          <span className="font-semibold opacity-60">Faculty Name</span><br />
                          {project.faculty || 'N/A'}
                        </div>
                        <div>
                          <span className="font-semibold opacity-60">Project Funding</span><br />
                          {project.source}
                        </div>
                        <div>
                          <span className="font-semibold opacity-60">Duration</span><br />
                          {project.duration || 'N/A'}
                        </div>
                      </div>
                      <p className="text-sm leading-relaxed whitespace-pre-line opacity-70">
                        {project.fullDescription}
                      </p>
                      <button onClick={() => setExpandedProjectId(null)} className="mt-4 text-white underline self-end font-anybody">CLOSE ✕</button>
                    </div>
                  </div>
                </div>
              ) : (
                ...
                >
                  <div className="p-5 flex flex-col justify-between h-full transition-colors duration-300 ease-in-out">
                    <img src={project.imageUrl} alt={project.title} className="w-full h-44 object-cover mb-4" />
                    <div className="flex flex-col justify-between grow">
                      <div>
                        <h3 className="font-semibold text-[15px] leading-snug transition-colors duration-300 ease-in-out font-anybody">{project.title}</h3>
